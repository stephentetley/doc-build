...
let WindowsEnv : DocBuildEnv = 
    { SourceDirectory   = @"G:\work\Projects\rtu\final-docs\input\mk3_mk4_upgrades"
      WorkingDirectory  = @"G:\work\Projects\rtu\final-docs\output\mk3_mk4_upgrades"
...
let genCoversheet (siteName:string) (saiNumber:string) : DocMonadWord<PdfDoc> = 
    ...
    let searches : SearchList = 
        [ ("#SAINUMBER", saiNumber); ("#SITENAME", siteName) ]
    docMonad { 
        let! (template:WordDoc) = getIncludeWordDoc "MK3_MK4 RTU Cover Sheet.docx"
...
let build1 (saiMap : SaiMap) : DocMonadWord<PdfDoc> = 
    docMonad { 
        ...
        let finalName = sprintf "%s S3953 Mk3 Mk4 MMIM Asset Upgrade.pdf" sourceName |> safeName
