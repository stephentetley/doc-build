...
type SiteTable = 
    ExcelFile< @"G:\work\Projects\rtu\final-docs\year3-batch2-manuals-todo.xlsx",
               SheetName = "TO_MAKE",
               ForceString = false >
...
let makeDict (row:SiteRow) : FindReplaceDict = 
    Map.ofList [ "#SITENAME", row.sitename
               ; "#SAINUM" , row.uid
               ]
...
let jsonConfig : FindsReplacesConfig<SiteRow> = 
    ...
    ; GetFileName       = 
        fun (row:SiteRow) -> sprintf "%s_findreplace.json" (safeName row.sitename)

    ; OutputJsonFolder = @"G:\work\Projects\rtu\final-docs\__Json" }
...
let batchConfig : BatchFileConfig = 
    { PathToFake = @"D:\coding\fsharp\DocMake\packages\FAKE.5.0.0-beta005\tools\FAKE.exe"
      ...
      BuildTarget = "Final"
      OutputBatchFile = @"G:\work\Projects\rtu\final-docs\fake-make.bat"
      VarName = "sitename"}
...
let main () : unit = 
    ...
    // Generate batch file...
    siteList 
        |> List.map (fun (row:SiteRow) -> row.sitename) 
